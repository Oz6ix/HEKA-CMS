...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Pathology Categories</h1>
            <p class="mt-1 text-sm text-slate-500">Organize pathology tests into logical categories.</p>
        </div>
        <div class="mt-4 sm:mt-0 flex items-center gap-3">
            ...
            <a href="{{ url($url_prefix . '/pathology_category/create') }}"
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-3 py-3 text-sm font-medium text-slate-900">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-folder text-violet-400 text-xs"></i>
                                    {{ $item->name }}
                                </div>
                            </td>
                            <td class="px-3 py-3 text-sm text-slate-600">{{ $item->subcategory->name ?? '-' }}</td>
                            <td class="px-3 py-3">
                                @if($item->status == 1)
                            ...
                            <td class="px-3 py-3 pr-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($item->status == 1)
                                        <a href="javascript:;" onclick="change_status('{{ url($url_prefix . '/pathology_category/deactivate/'.$item->id) }}')" class="text-slate-400 hover:text-amber-500 transition-colors" title="Deactivate">
                                            <i class="fas fa-toggle-on text-emerald-500"></i>
                                        </a>
                                    @else
                                        <a href="javascript:;" onclick="change_status('{{ url($url_prefix . '/pathology_category/activate/'.$item->id) }}')" class="text-slate-400 hover:text-emerald-500 transition-colors" title="Activate">
                                            <i class="fas fa-toggle-off"></i>
                                        </a>
                                    @endif
                                    <a href="{{ url($url_prefix . '/pathology_category/edit/'.$item->id) }}" class="text-slate-400 hover:text-primary-600 transition-colors" title="Edit">
                                        <i class="fas fa-pen-to-square"></i>
                                    </a>
                                    <a href="javascript:;" onclick="delete_record('{{ url($url_prefix . '/pathology_category/delete/'.$item->id) }}')" class="text-slate-400 hover:text-red-500 transition-colors" title="Delete">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="text-slate-400">
                                    ...
                                    <p class="text-xs mt-1">Add your first category to organize tests.</p>
                                </div>
                            </td>
                        </tr>
                    @endif
