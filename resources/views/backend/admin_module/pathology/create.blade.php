...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Add Pathology Test</h1>
            <p class="mt-1 text-sm text-slate-500">Create a new pathology test with parameters and pricing.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {!! Form::open(['route'=>'pathology_add', 'id' => 'add_form', 'class' => 'kt-form', 'files' => true, 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="test" id="test" value="{{ old('test') }}" placeholder="e.g. Fasting Blood Sugar"
                ...
                <div>
                    ...
                    <input type="hidden" name="pathology_category_id" id="pathology_category_id" value="{{ old('pathology_category_id') }}">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Report Days <span class="text-red-500">*</span></label>
                    <input type="text" name="report_days" id="report_days" value="{{ old('report_days') }}" placeholder="e.g. 2"
                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Charges (K) <span class="text-red-500">*</span></label>
                    <input type="text" name="charge" id="charge" value="{{ old('charge') }}" placeholder="e.g. 5000"
                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1.5">Note</label>
                <textarea name="note" id="note" rows="3" placeholder="Additional notes about this test..."
