...
@section('content')
    ...
    <div class="max-w-7xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ route('pharmacy_category_update') }}" method="POST" id="update_form" class="space-y-6" enctype="multipart/form-data">
                @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Parent Category -->
                <div>
                    <label for="parent_id" class="block mb-2 text-sm font-medium text-gray-900">Parent Category</label>
                    <select name="parent_id" id="parent_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                        <option value="0">None (Top Level)</option>
                        @foreach($parent_category as $data)
                            <option value="{{ $data['id'] }}" {{ ($data['id'] == $item['parent_id']) ? 'selected' : '' }}>
                                {{ $data['name'] }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
                <a href="{{ url($url_prefix . '/pharmacy_categorys') }}" class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5">
                    Cancel
                </a>
