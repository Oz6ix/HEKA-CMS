...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Hospital Charges</h1>
            <p class="mt-1 text-sm text-slate-500">Manage Hospital Charges records.</p>
        </div>
        <div class="mt-4 sm:mt-0 flex items-center gap-3">
            <a href="{{ url($url_prefix . '/hospital_charge_categorys') }}"
            ...
            <a href="{{ url($url_prefix . '/hospital_charge/create') }}"
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Title</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Rate (K)</th>
                    ...
                    @if(isset($items) && sizeof($items) > 0)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-3 py-3 text-sm text-slate-600">
                                <span class="inline-flex items-center rounded-full bg-violet-50 px-2.5 py-0.5 text-xs font-medium text-violet-700">{{ ($item->hospital_charge_category->name ?? '-') }}</span>
                            </td>
                            <td class="px-3 py-3 text-sm font-medium text-slate-900">{{ $item->title }}</td>
                            <td class="px-3 py-3 text-sm font-semibold text-slate-700">{{ number_format($item->standard_charge) }}</td>
                            <td class="px-3 py-3">
                                @if($item->status == 1)
                            ...
                            <td class="px-3 py-3 pr-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($item->status == 1)
                                        <a href="javascript:;" onclick="change_status('{{ url($url_prefix . '/hospital_charge/deactivate/'.$item->id) }}')" class="text-slate-400 hover:text-amber-500 transition-colors" title="Deactivate">
                                            <i class="fas fa-toggle-on text-emerald-500"></i>
                                        </a>
                                    @else
                                        <a href="javascript:;" onclick="change_status('{{ url($url_prefix . '/hospital_charge/activate/'.$item->id) }}')" class="text-slate-400 hover:text-emerald-500 transition-colors" title="Activate">
                                            <i class="fas fa-toggle-off"></i>
                                        </a>
                                    @endif
                                    <a href="{{ url($url_prefix . '/hospital_charge/edit/'.$item->id) }}" class="text-slate-400 hover:text-primary-600 transition-colors" title="Edit">
                                        <i class="fas fa-pen-to-square"></i>
                                    </a>
                                    <a href="javascript:;" onclick="delete_record('{{ url($url_prefix . '/hospital_charge/delete/'.$item->id) }}')" class="text-slate-400 hover:text-red-500 transition-colors" title="Delete">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-slate-400">
                                    ...
                                    <p class="text-xs mt-1">Add your first Charge to get started.</p>
                                </div>
                            </td>
                        </tr>
                    @endif
