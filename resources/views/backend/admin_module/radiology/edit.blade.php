...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Radiology Test</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {!! Form::open(['route'=>'radiology_update', 'id' => 'update_form', 'class' => 'kt-form', 'files' => true, 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="test" id="test" value="{{ $item['test'] }}" placeholder="e.g. X-Ray Chest PA"
                ...
                <div>
                    ...
                    <select id="select2_radiology_category_id" class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                        ...
                        @foreach($radiology_category as $data)
                            <option value="{{ $data['id'] }}" {{ ($data['id'] == $item['radiology_category_id']) ? 'selected' : '' }}>{{ $data['name'] }}</option>
                        @endforeach
                    </select>
                    <input type="hidden" name="radiology_category_id" id="radiology_category_id" value="{{ $item['radiology_category_id'] }}">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Report Days <span class="text-red-500">*</span></label>
                    <input type="text" name="report_days" id="report_days" value="{{ $item['report_days'] }}" placeholder="e.g. 1"
                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Charges (K) <span class="text-red-500">*</span></label>
                    <input type="text" name="charge" id="charge" value="{{ $item['charge'] }}" placeholder="e.g. 10000"
                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                </div>
            </div>
            <div>
            ...
            <div id="kt_repeater_1">
                <div class="form-group form-group-last row" id="kt_repeater_1">
                    <div data-repeater-list="parameters" class="space-y-3 w-full">
                        @if(isset($radiology_parameters) && !empty($radiology_parameters) && count($radiology_parameters) > 0)
                            @foreach($radiology_parameters as $parameter_value)
                            <div data-repeater-item class="flex items-end gap-3">
                                <div class="flex-1">
                                    ...
                                    <input type="text" name="parameter_name" required placeholder="e.g. Contrast" value="{{ $parameter_value['parameter_name'] }}"
                                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                                ...
                                <div class="flex-1">
                                    ...
                                    <input type="text" name="range" required placeholder="e.g. Normal" value="{{ $parameter_value['range'] }}"
                                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                                </div>
                                <div class="flex-1">
                                    <label class="block text-xs font-medium text-slate-600 mb-1">Unit</label>
                                    <input type="text" name="unit" required placeholder="e.g. mm" value="{{ $parameter_value['unit'] }}"
                                        class="block w-full rounded-lg border-slate-300 text-sm focus:ring-primary-500 focus:border-primary-500">
                                </div>
