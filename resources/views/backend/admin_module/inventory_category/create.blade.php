...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        ...
        <a href="{{ url($url_prefix . '/inventory_categorys') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl">
        {!! Form::open(['route'=>'inventory_category_add', 'id' => 'add_form', 'class' => 'kt-form', 'files' => true, 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
        <div class="p-6 space-y-5">
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1.5">Category Name <span class="text-red-500">*</span></label>
                <input type="text" name="inventory_name" id="inventory_name" value="{{ old('inventory_name') }}" placeholder="Enter Category Name" 
        ...
        <div class="border-t border-slate-200 px-6 py-4 bg-slate-50 flex items-center justify-end gap-3">
            <a href="{{ url($url_prefix . '/inventory_categorys') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white transition-colors">Cancel</a>
...
@section('scripts')
<script src="{{ URL::asset('assets/backend/js/validations/inventory_category.js') }}" type="text/javascript"></script>
@endsection
