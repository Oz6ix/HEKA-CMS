...
@section('content')
    ...
    <div class="max-w-7xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ route('pharmacy_add') }}" method="POST" id="add_form" class="space-y-6" enctype="multipart/form-data">
                @csrf
                ...
                <div>
                    ...
                    <select name="pharmacy_category_id" id="pharmacy_category_id" 
                        ...
                        @foreach($hospital_category as $data)
                            <option value="{{ $data['id'] }}" {{ old('pharmacy_category_id') == $data['id'] ? 'selected' : '' }}>
                                {{ $data['name'] }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-gray-100">
                ...
                <div>
                    ...
                    <select name="medicine_type" id="medicine_type" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                        <option value="allopathy" {{ old('medicine_type', 'allopathy') == 'allopathy' ? 'selected' : '' }}>Allopathy</option>
                        <option value="ayurveda" {{ old('medicine_type') == 'ayurveda' ? 'selected' : '' }}>Ayurveda</option>
                        <option value="nutrition" {{ old('medicine_type') == 'nutrition' ? 'selected' : '' }}>Nutrition / Supplement</option>
                        <option value="psychology" {{ old('medicine_type') == 'psychology' ? 'selected' : '' }}>Psychology</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="schedule" id="schedule" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                        <option value="OTC" {{ old('schedule', 'OTC') == 'OTC' ? 'selected' : '' }}>OTC (Over-the-Counter)</option>
                        <option value="H" {{ old('schedule') == 'H' ? 'selected' : '' }}>Schedule H</option>
                        <option value="H1" {{ old('schedule') == 'H1' ? 'selected' : '' }}>Schedule H1</option>
                        <option value="X" {{ old('schedule') == 'X' ? 'selected' : '' }}>Schedule X (Narcotics)</option>
                    </select>
                </div>
                ...
                <div class="flex items-end">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_generic" value="1" class="sr-only peer" {{ old('is_generic') ? 'checked' : '' }}>
                ...
                <!-- Generic Group (link to generic equivalent) -->
                <div>
                    ...
                    <select name="generic_group_id" id="generic_group_id" 
                        ...
                        @if(isset($generic_drugs))
                            @foreach($generic_drugs as $gd)
                                <option value="{{ $gd->id }}" {{ old('generic_group_id') == $gd->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-gray-100">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="mrp" id="mrp" value="{{ old('mrp') }}" step="0.01"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 pr-12"
                               placeholder="0.00">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">Ks</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                ...
                <div>
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="photo" class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6" id="photo_preview_container">
                            ...
                            <img id="thumbnail_image" class="hidden h-40 object-contain" src="#" alt="Image Preview" />
                ...
                <div>
                    ...
                    <textarea name="note" id="note" rows="7" 
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" 
                              placeholder="Optional notes about usage, side effects, storage instructions...">{!! old('note') !!}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" id="add_button" class="text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5">
                    <i class="fas fa-check mr-2"></i> Save Medicine
                </button>
            </div>
